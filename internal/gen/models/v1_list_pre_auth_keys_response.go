// Code generated by go-swagger; DO NOT EDIT.

package models

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"context"
	"strconv"

	"github.com/go-openapi/errors"
	"github.com/go-openapi/strfmt"
	"github.com/go-openapi/swag"
)

// V1ListPreAuthKeysResponse v1 list pre auth keys response
//
// swagger:model v1ListPreAuthKeysResponse
type V1ListPreAuthKeysResponse struct {

	// pre auth keys
	PreAuthKeys []*V1PreAuthKey `json:"preAuthKeys"`
}

// Validate validates this v1 list pre auth keys response
func (m *V1ListPreAuthKeysResponse) Validate(formats strfmt.Registry) error {
	var res []error

	if err := m.validatePreAuthKeys(formats); err != nil {
		res = append(res, err)
	}

	if len(res) > 0 {
		return errors.CompositeValidationError(res...)
	}
	return nil
}

func (m *V1ListPreAuthKeysResponse) validatePreAuthKeys(formats strfmt.Registry) error {
	if swag.IsZero(m.PreAuthKeys) { // not required
		return nil
	}

	for i := 0; i < len(m.PreAuthKeys); i++ {
		if swag.IsZero(m.PreAuthKeys[i]) { // not required
			continue
		}

		if m.PreAuthKeys[i] != nil {
			if err := m.PreAuthKeys[i].Validate(formats); err != nil {
				if ve, ok := err.(*errors.Validation); ok {
					return ve.ValidateName("preAuthKeys" + "." + strconv.Itoa(i))
				} else if ce, ok := err.(*errors.CompositeError); ok {
					return ce.ValidateName("preAuthKeys" + "." + strconv.Itoa(i))
				}
				return err
			}
		}

	}

	return nil
}

// ContextValidate validate this v1 list pre auth keys response based on the context it is used
func (m *V1ListPreAuthKeysResponse) ContextValidate(ctx context.Context, formats strfmt.Registry) error {
	var res []error

	if err := m.contextValidatePreAuthKeys(ctx, formats); err != nil {
		res = append(res, err)
	}

	if len(res) > 0 {
		return errors.CompositeValidationError(res...)
	}
	return nil
}

func (m *V1ListPreAuthKeysResponse) contextValidatePreAuthKeys(ctx context.Context, formats strfmt.Registry) error {

	for i := 0; i < len(m.PreAuthKeys); i++ {

		if m.PreAuthKeys[i] != nil {

			if swag.IsZero(m.PreAuthKeys[i]) { // not required
				return nil
			}

			if err := m.PreAuthKeys[i].ContextValidate(ctx, formats); err != nil {
				if ve, ok := err.(*errors.Validation); ok {
					return ve.ValidateName("preAuthKeys" + "." + strconv.Itoa(i))
				} else if ce, ok := err.(*errors.CompositeError); ok {
					return ce.ValidateName("preAuthKeys" + "." + strconv.Itoa(i))
				}
				return err
			}
		}

	}

	return nil
}

// MarshalBinary interface implementation
func (m *V1ListPreAuthKeysResponse) MarshalBinary() ([]byte, error) {
	if m == nil {
		return nil, nil
	}
	return swag.WriteJSON(m)
}

// UnmarshalBinary interface implementation
func (m *V1ListPreAuthKeysResponse) UnmarshalBinary(b []byte) error {
	var res V1ListPreAuthKeysResponse
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*m = res
	return nil
}
